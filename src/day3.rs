...
pub fn parse_and_compute<const ENABLE_DO_STATE: bool>(input: &[u8]) -> usize {
  ...
  'outer: loop {
    ...
    } else if ENABLE_DO_STATE && do_state && input.get(char_ix..char_ix + DONT.len()) == Some(&DONT)
    {
      do_state = false;
      char_ix += DONT.len();
    } else if ENABLE_DO_STATE && !do_state && input.get(char_ix..char_ix + DO.len()) == Some(&DO) {
      do_state = true;
      char_ix += DO.len();
    } else {
      char_ix += 1;
